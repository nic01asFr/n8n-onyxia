...
- **Connexion à n8n.** Pour fonctionner, le portail est connecté à n8n comme le serveur MCP : par la clé API créée au démarrage, sans rien demander. Pour ouvrir le mode « Configurer », le propriétaire prouve qu'il tient ce n8n : avec le jeton du serveur MCP (notes Onyxia, proposé en premier quand le MCP est activé), avec le compte du service (email, mot de passe, code de double authentification s'il y en a), vérifié par n8n sur `/rest/login` puis refermé, ou avec une clé API. Seuls le propriétaire et les administrateurs de l'instance passent. Il en reste une session d'une heure liée au compte Grist ; rien n'est conservé. Le premier compte Grist connecté qui s'identifie devient propriétaire du portail.
- **Éditeur d'action.** Présentation, étapes et champs (texte, nombre, date, listes, cases, oui/non, échelle), conditions d'affichage, clés du résultat et leur rendu, accès au document et table du résultat ; un mode JSON reste ouvert pour le FormDef complet. Il propose les champs que le webhook lit déjà. Son aperçu lance le workflow pour de vrai (`POST /api/admin/try`, propriétaire seulement, `_portail.test: true`, avec l'accès au document réglé) et montre tout le retour, dont les clés s'ajoutent au résultat en un clic.
